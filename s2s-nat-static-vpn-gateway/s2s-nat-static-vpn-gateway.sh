...
az network vnet-gateway create -g $rg -n $hub1_vnet_name-gw --public-ip-addresses $hub1_vnet_name-gw0 $hub1_vnet_name-gw1 --vnet $hub1_vnet_name --sku VpnGw2AZ --vpn-gateway-generation Generation2 --vpn-gateway-generation Generation2 --gateway-type Vpn --vpn-type RouteBased --no-wait
...
case "${PLUTO_VERB}" in
    up-client)
        ...
        ip link set "${VTI_INTERFACE}" up
        ip route add $hub1_nat_address nexthop dev "${VTI_INTERFACE}" nexthop dev "${O_VTI_IF}"
        ip route add $onprem1_nat_address nexthop dev "${VTI_INTERFACE}" nexthop dev "${O_VTI_IF}"
        sysctl -w "net.ipv4.conf.${VTI_INTERFACE}.disable_policy=1"
        ;;
    down-client)
        $IP link del "${VTI_INTERFACE}"
        ip tunnel del "${VTI_INTERFACE}"
        ip route delete $hub1_nat_address
        ip route delete $onprem1_nat_address
        $IPTABLES -t mangle -D FORWARD -o ${VTI_INTERFACE} -p tcp -m tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
        $IPTABLES -t mangle -D INPUT -p esp -s ${PLUTO_PEER} -d ${PLUTO_ME} -j MARK --set-xmark ${PLUTO_MARK_IN}
        ;;
esac
