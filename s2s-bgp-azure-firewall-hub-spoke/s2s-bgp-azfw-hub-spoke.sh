...
az network vnet subnet create -g $rg -n AzureFirewallSubnet --address-prefixes $hub1_fw_subnet_address --vnet-name $hub1_vnet_name -o none
...
az vm create -g $rg -n $onprem1_vnet_name-gw -l $location1 --image $vm_image --nics $onprem1_vnet_name-gw --os-disk-name $onprem1_vnet_name-gw --size $vm_size --admin-username $admin_username --generate-ssh-keys --security-type TrustedLaunch --custom-data $cloudinit_file --no-wait
# onprem1 gw details
...
az vm create -g $rg -n $onprem2_vnet_name-gw -l $location2 --image $vm_image --nics $onprem2_vnet_name-gw --os-disk-name $onprem2_vnet_name-gw --size $vm_size --admin-username $admin_username --generate-ssh-keys --security-type TrustedLaunch --custom-data $cloudinit_file --no-wait
# onprem2 gw details
...
az vm create -g $rg -n $onprem1_vnet_name -l $location1 --image $vm_image --nics $onprem1_vnet_name --os-disk-name $onprem1_vnet_name --size $vm_size --admin-username $admin_username --generate-ssh-keys --security-type TrustedLaunch --no-wait
...
az vm create -g $rg -n $onprem2_vnet_name -l $location2 --image $vm_image --nics $onprem2_vnet_name --os-disk-name $onprem2_vnet_name --size $vm_size --admin-username $admin_username --generate-ssh-keys --security-type TrustedLaunch --no-wait
...
az vm create -g $rg -n $hub1_vnet_name -l $location1 --image $vm_image --nics $hub1_vnet_name --os-disk-name $hub1_vnet_name --size $vm_size --admin-username $admin_username --generate-ssh-keys --security-type TrustedLaunch --no-wait
...
az vm create -g $rg -n $spoke1_vnet_name -l $location1 --image $vm_image --nics $spoke1_vnet_name --os-disk-name $spoke1_vnet_name --size $vm_size --admin-username $admin_username --generate-ssh-keys --security-type TrustedLaunch --no-wait
...
az vm create -g $rg -n $spoke2_vnet_name -l $location2 --image $vm_image --nics $spoke2_vnet_name --os-disk-name $spoke2_vnet_name --size $vm_size --admin-username $admin_username --generate-ssh-keys --security-type TrustedLaunch --no-wait
spoke2_vm_ip=$(az network nic show -g $rg -n $spoke2_vnet_name --query ipConfigurations[0].privateIPAddress -o tsv) && echo $spoke2_vnet_name vm private ip: $spoke2_vm_ip
...
az network firewall policy create -g $rg -n $hub1_vnet_name-fw-policy -l $location1 -o none
...
az network firewall policy rule-collection-group collection add-filter-collection -g $rg -n $hub1_vnet_name-NetworkRuleCollection --policy-name $hub1_vnet_name-fw-policy --rcg-name $hub1_vnet_name-RuleCollectionGroup --action Allow --rule-name cloud-to-cloud-traffic --collection-priority 100 --rule-type NetworkRule --source-addresses $spoke1_vnet_address $spoke2_vnet_address $hub1_vnet_address --ip-protocols any --destination-addresses $spoke1_vnet_address $spoke2_vnet_address $hub1_vnet_address --destination-ports '*' -o none
az network firewall policy rule-collection-group collection rule add -g $rg -n cloud-to-onprem-traffic --policy-name $hub1_vnet_name-fw-policy --rule-collection-group-name $hub1_vnet_name-RuleCollectionGroup  --collection-name $hub1_vnet_name-NetworkRuleCollection --rule-type NetworkRule --dest-addr $onprem1_vnet_address $onprem2_vnet_address --destination-ports '*' --ip-protocols any --source-addresses $spoke1_vnet_address $spoke2_vnet_address $hub1_vnet_address -o none
az network firewall policy rule-collection-group collection rule add -g $rg -n onprem-to-cloud-traffic --policy-name $hub1_vnet_name-fw-policy --rule-collection-group-name $hub1_vnet_name-RuleCollectionGroup  --collection-name $hub1_vnet_name-NetworkRuleCollection --rule-type NetworkRule --dest-addr $spoke1_vnet_address $spoke2_vnet_address $hub1_vnet_address --destination-ports '*' --ip-protocols any --source-addresses $onprem1_vnet_address $onprem2_vnet_address -o none
az network firewall policy rule-collection-group collection add-filter-collection -g $rg -n $hub1_vnet_name-ApplicationRuleCollection --policy-name $hub1_vnet_name-fw-policy --rule-collection-group-name $hub1_vnet_name-RuleCollectionGroup --action Allow --rule-name AllowHttps --collection-priority 300 --rule-type ApplicationRule --protocols http=80 https=443 --source-addresses "*" --source-addresses "*" --destination-fqdns "*" --target-fqdns "*" --description "Allow http/https" -o none

# hub1 azure firewall
echo -e "\e[1;36mCreating $hub1_vnet_name-fw Azure Firewall....\e[0m"
az network public-ip create -g $rg -n $hub1_vnet_name-fw -l $location1 --allocation-method Static --sku Standard -o none
az network firewall create -g $rg -n $hub1_vnet_name-fw -l $location1 --sku AZFW_VNet --firewall-policy $hub1_vnet_name-fw-policy -o none
...
hub1_gw_pubip0=$(az network vnet-gateway show -g $rg -n $hub1_vnet_name-gw --query 'bgpSettings.bgpPeeringAddresses[0].tunnelIpAddresses[0]' -o tsv) && echo $hub1_vnet_name-gw: $hub1_gw_pubip0
hub1_gw_pubip1=$(az network vnet-gateway show -g $rg -n $hub1_vnet_name-gw --query 'bgpSettings.bgpPeeringAddresses[1].tunnelIpAddresses[0]' -o tsv) && echo $hub1_vnet_name-gw: $hub1_gw_pubip1
hub1_gw_bgp_ip0=$(az network vnet-gateway show -g $rg -n $hub1_vnet_name-gw --query 'bgpSettings.bgpPeeringAddresses[0].defaultBgpIpAddresses[0]' -o tsv) && echo $hub1_vnet_name-gw: $hub1_gw_bgp_ip0
hub1_gw_bgp_ip1=$(az network vnet-gateway show -g $rg -n $hub1_vnet_name-gw --query 'bgpSettings.bgpPeeringAddresses[1].defaultBgpIpAddresses[0]' -o tsv) && echo $hub1_vnet_name-gw: $hub1_gw_bgp_ip1
hub1_gw_asn=$(az network vnet-gateway show -g $rg -n $hub1_vnet_name-gw --query bgpSettings.asn -o tsv) && echo $hub1_vnet_name-gw: $hub1_gw_asn
...
#############################################################
echo -e "\e[1;36mRestarting IPSEC on $onprem1_vnet_name-gw gateway vm...\e[0m"
...
echo -e "\e[1;36mRestarting IPSEC on $onprem2_vnet_name-gw gateway vm...\e[0m"
ssh -n -o BatchMode=yes -o StrictHostKeyChecking=no $onprem2_vnet_name "sudo ipsec stop && sudo ipsec start && sudo ipsec status && ip a"
